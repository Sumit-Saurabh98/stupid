...
export const signUp = async (req:Request, res:Response) => {
    ...
    try {
        ...
        const {accessToken} = generateToken({_id:newUser._id.toString(), username:newUser.username});

        setCookie(res, accessToken);

        res.status(200).json({message: "Account created successfully", data: newUser, status: true});
...
export const login = async (req:Request, res:Response) => {
    ...
    try {
        ...
        const {accessToken} = generateToken({_id:user._id.toString(), username:user.username})
